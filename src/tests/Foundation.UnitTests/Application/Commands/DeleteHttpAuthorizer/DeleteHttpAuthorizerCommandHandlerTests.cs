...
namespace Foundation.UnitTests.Application.Commands.DeleteHttpAuthorizer;

public class DeleteHttpAuthorizerCommandHandlerTests
{
    ...
    private DeleteHttpAuthorizerCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteHttpAuthorizerCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        ...
        var result = await sut.Handle(
            new DeleteHttpAuthorizerCommand("abc123", "auth1"), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).DeleteAuthorizerAsync("abc123", "auth1", Arg.Any<CancellationToken>());

using AspNet.KickStarter.FunctionalResult;
using Foundation.Application.EventBridge;
using Foundation.Application.Queries.ListEventBridgeRules;
using Foundation.Domain.EventBridge;
using Microsoft.Extensions.Logging.Abstractions;

namespace Foundation.UnitTests.Application.Queries.ListEventBridgeRules;

public class ListEventBridgeRulesQueryHandlerTests
{
    private readonly IEventBridgeClient _client = Substitute.For<IEventBridgeClient>();

    private ListEventBridgeRulesQueryHandler CreateSut()
        => new(_client, NullLogger<ListEventBridgeRulesQueryHandler>.Instance);

    private static Result<T> Ok<T>(T value)
        => value;

    [Fact]
    public async Task Handle_WhenClientSucceeds_ReturnsRules()
    {
        // Arrange
        IReadOnlyList<EventBridgeRule> rules =
        [
            new(
                "orders-rule",
                "arn:aws:events:eu-west-1:000000000000:rule/orders-rule",
                "default",
                "ENABLED",
                "Routes order events",
                null),
        ];
        _client
            .ListRulesAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Ok(rules)));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new ListEventBridgeRulesQuery(), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Rules.Should().ContainSingle(_ => _.Name == "orders-rule");
    }

    [Fact]
    public async Task Handle_WhenClientFails_PropagatesError()
    {
        // Arrange
        _client
            .ListRulesAsync(Arg.Any<CancellationToken>())
            .Returns(Task.FromResult<Result<IReadOnlyList<EventBridgeRule>>>(new Error("boom")));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new ListEventBridgeRulesQuery(), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeFalse();
        result.Error!.Value.Message.Should().Be("boom");
    }
}

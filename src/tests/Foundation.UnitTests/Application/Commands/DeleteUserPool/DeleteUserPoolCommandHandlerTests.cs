...
public class DeleteUserPoolCommandHandlerTests
{
    ...
    private DeleteUserPoolCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteUserPoolCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteUserPoolAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteUserPoolAsync("eu-west-1_abc123", Arg.Any<CancellationToken>());

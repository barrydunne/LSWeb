...
public class CreateLogGroupCommandHandlerTests
{
    ...
    private CreateLogGroupCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreateLogGroupCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreateLogGroupAsync("/app/orders", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new CreateLogGroupCommand("/app/orders"),
            TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).CreateLogGroupAsync("/app/orders", Arg.Any<CancellationToken>());

...
public class PutRoleInlinePolicyCommandHandlerTests
{
    ...
    private static PutRoleInlinePolicyCommand BuildCommand()
        => new("deploy-role", "inline", PolicyDocument);

    private PutRoleInlinePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<PutRoleInlinePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenPutSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .PutRoleInlinePolicyAsync("deploy-role", "inline", PolicyDocument, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).PutRoleInlinePolicyAsync(
            "deploy-role", "inline", PolicyDocument, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenPutFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .PutRoleInlinePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

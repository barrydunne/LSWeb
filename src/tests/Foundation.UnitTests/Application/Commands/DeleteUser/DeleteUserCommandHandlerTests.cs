...
public class DeleteUserCommandHandlerTests
{
    ...
    private static DeleteUserCommand BuildCommand()
        => new("alice");

    private DeleteUserCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteUserCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteUserAsync("alice", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteUserAsync("alice", Arg.Any<CancellationToken>());

...
public class DeleteChangeSetCommandHandlerTests
{
    ...
    private DeleteChangeSetCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh,
            NullLogger<DeleteChangeSetCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteChangeSetAsync("orders-stack", "add-queue", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteChangeSetCommand("orders-stack", "add-queue"),
            TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).DeleteChangeSetAsync(
    ...
    public async Task Handle_WhenDeleteFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteChangeSetAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

...
public class PutUserInlinePolicyCommandHandlerTests
{
    ...
    private static PutUserInlinePolicyCommand BuildCommand()
        => new("alice", "inline", PolicyDocument);

    private PutUserInlinePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<PutUserInlinePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .PutUserInlinePolicyAsync("alice", "inline", PolicyDocument, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).PutUserInlinePolicyAsync(
            "alice", "inline", PolicyDocument, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .PutUserInlinePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

...
namespace Foundation.UnitTests.Application.Commands.DeleteRestResource;

public class DeleteRestResourceCommandHandlerTests
{
    ...
    private DeleteRestResourceCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteRestResourceCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteResourceAsync("api-1", "res-2", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteRestResourceCommand("api-1", "res-2"), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).DeleteResourceAsync("api-1", "res-2", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenDeleteFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteResourceAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

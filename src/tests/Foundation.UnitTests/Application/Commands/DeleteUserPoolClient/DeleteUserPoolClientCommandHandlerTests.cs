...
public class DeleteUserPoolClientCommandHandlerTests
{
    ...
    private DeleteUserPoolClientCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteUserPoolClientCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteUserPoolClientAsync("eu-west-1_abc123", "client-1", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteUserPoolClientCommand("eu-west-1_abc123", "client-1"), TestContext.Current.CancellationToken);
    ...
    public async Task Handle_WhenDeleteFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteUserPoolClientAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

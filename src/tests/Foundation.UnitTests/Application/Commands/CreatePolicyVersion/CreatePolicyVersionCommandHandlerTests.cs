...
public class CreatePolicyVersionCommandHandlerTests
{
    ...
    private static CreatePolicyVersionCommand BuildCommand()
        => new(Arn, Document, true);

    private CreatePolicyVersionCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreatePolicyVersionCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreatePolicyVersionAsync(Arn, Document, true, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).CreatePolicyVersionAsync(Arn, Document, true, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenCreateFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .CreatePolicyVersionAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<bool>(), Arg.Any<CancellationToken>())

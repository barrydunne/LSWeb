...
public class DetachGroupPolicyCommandHandlerTests
{
    ...
    private static DetachGroupPolicyCommand BuildCommand()
        => new("developers", "arn:aws:iam::aws:policy/ReadOnlyAccess");

    private DetachGroupPolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DetachGroupPolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DetachGroupPolicyAsync("developers", "arn:aws:iam::aws:policy/ReadOnlyAccess", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DetachGroupPolicyAsync(
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DetachGroupPolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

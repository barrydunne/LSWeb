...
describe('getApiGatewayRestCors', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('returns the parsed CORS configuration when the request succeeds', async () => {
    const payload = {
      resourceId: 'res2',
      enabled: true,
      allowOrigins: ['*'],
    ...
    const result = await getApiGatewayRestCors('api 1', 'res 2', controller.signal);

    expect(result.enabled).toBe(true);
    expect(result.allowMethods).toEqual(['GET', 'POST']);
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/apigateway/restapis/api%201/resources/res%202/cors',
      { signal: controller.signal },
    );
  });
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(getApiGatewayRestCors('api1', 'missing')).rejects.toThrow(
      'API Gateway CORS request failed with status 404',
    );
  });
});

describe('configureApiGatewayRestCors', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('puts the CORS configuration with the encoded ids', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
    ...
    await configureApiGatewayRestCors('api 1', 'res 2', request, controller.signal);
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      configureApiGatewayRestCors('api1', 'res1', {
        allowOrigins: ['*'],
        allowMethods: ['GET'],
        allowHeaders: ['Content-Type'],
      }),
    ).rejects.toThrow('API Gateway CORS configure request failed with status 400');
  });
});
...
describe('testInvokeApiGatewayRestMethod', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('posts the test invoke payload to the encoded endpoint and returns the payload', async () => {
    const payload = {
      statusCode: 200,
    ...
    const result = await testInvokeApiGatewayRestMethod(
      'api 1',
      'res 2',
      'POST',
      request,
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/apigateway/restapis/api%201/resources/res%202/methods/POST/test-invoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(request),
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      testInvokeApiGatewayRestMethod('api1', 'res2', 'GET', {
        pathWithQueryString: '/orders',
        headers: {},
    ...
    ).rejects.toThrow('API Gateway method test invoke request failed with status 500');
  });
});
...
describe('createApiGatewayRestTokenAuthorizer', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('posts the token authorizer create request and returns the created id', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ ok: true, json: async () => ({ id: 'auth7' }) });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
      name: 'jwt-authorizer',
    ...
    const result = await createApiGatewayRestTokenAuthorizer('api 1', request, controller.signal);

    expect(result.id).toBe('auth7');
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/apigateway/restapis/api%201/authorizers/token',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(request),
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      createApiGatewayRestTokenAuthorizer('api1', {
        name: 'jwt-authorizer',
        issuer: 'https://issuer.example.com',
        audience: 'api://default',
        identitySource: 'method.request.header.Authorization',
    ...
    ).rejects.toThrow('API Gateway token authorizer create request failed with status 400');
  });
});
...
describe('getUserPools', () => {
  ...
  it('returns the parsed user pools when the request succeeds', async () => {
    const payload = {
      userPools: [
        {
          id: 'eu-west-1_abc123',
    ...
    const result = await getUserPools(controller.signal);

    expect(result.userPools).toHaveLength(1);
    expect(result.userPools[0].id).toBe('eu-west-1_abc123');
    expect(fetchMock).toHaveBeenCalledWith('/api/services/cognito/user-pools', {
      signal: controller.signal,
    });
  });
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(getUserPools()).rejects.toThrow(
      'Cognito user pools request failed with status 503',
    );
  });
});
...
describe('getUserPool', () => {
  ...
  it('returns the parsed user pool with the encoded id when the request succeeds', async () => {
    const payload = {
      id: 'eu-west-1_abc 123',
      name: 'customers',
    ...
    const result = await getUserPool('eu-west-1_abc 123', controller.signal);

    expect(result.name).toBe('customers');
    expect(result.mfaConfiguration).toBe('OPTIONAL');
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/cognito/user-pools/eu-west-1_abc%20123',
      { signal: controller.signal },
    );
  });
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(getUserPool('missing')).rejects.toThrow(
      'Cognito user pool request failed with status 404',
    );
  });
});
...
describe('createUserPool', () => {
  ...
  it('posts the user pool create request and returns the created id', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue({ ok: true, json: async () => ({ id: 'eu-west-1_new' }) });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
      name: 'customers',
    ...
    expect(result.id).toBe('eu-west-1_new');
    expect(fetchMock).toHaveBeenCalledWith('/api/services/cognito/user-pools', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(request),
...
describe('deleteUserPool', () => {
  ...
  it('deletes the user pool with the encoded id', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();

    await deleteUserPool('eu-west-1_abc 123', controller.signal);
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(deleteUserPool('missing')).rejects.toThrow(
      'Cognito user pool delete request failed with status 404',
    );
  });
});

describe('getUserPoolClients', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('returns the parsed clients with the encoded pool id when the request succeeds', async () => {
    const payload = {
      clients: [
        {
          clientId: 'client-1',
    ...
    const result = await getUserPoolClients('eu-west-1_abc 123', controller.signal);

    expect(result.clients).toHaveLength(1);
    expect(result.clients[0].clientId).toBe('client-1');
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/cognito/user-pools/eu-west-1_abc%20123/clients',
      { signal: controller.signal },
    );
  });
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(getUserPoolClients('eu-west-1_abc123')).rejects.toThrow(
      'Cognito user pool clients request failed with status 503',
    );
  });
});

describe('getUserPoolClient', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('returns the parsed client with encoded ids when the request succeeds', async () => {
    const payload = {
      clientId: 'client 1',
      clientName: 'web',
      userPoolId: 'eu-west-1_abc 123',
      clientSecret: 'secret',
      generateSecret: true,
    ...
    const result = await getUserPoolClient('eu-west-1_abc 123', 'client 1', controller.signal);

    expect(result.clientName).toBe('web');
    expect(result.clientSecret).toBe('secret');
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/cognito/user-pools/eu-west-1_abc%20123/clients/client%201',
      { signal: controller.signal },
    );
  });
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(getUserPoolClient('eu-west-1_abc123', 'missing')).rejects.toThrow(
      'Cognito user pool client request failed with status 404',
    );
  });
});
...
describe('createUserPoolClient', () => {
  ...
  it('posts the client create request and returns the created client', async () => {
    const payload = {
      clientId: 'client-1',
      clientName: 'web',
      userPoolId: 'eu-west-1_abc123',
    ...
    const result = await createUserPoolClient('eu-west-1_abc 123', request, controller.signal);
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      createUserPoolClient('eu-west-1_abc123', {
        clientName: 'web',
    ...
    ).rejects.toThrow('Cognito user pool client create request failed with status 400');
  });
});

describe('updateUserPoolClient', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('puts the client update request with encoded ids', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
    ...
    await updateUserPoolClient('eu-west-1_abc 123', 'client 1', request, controller.signal);
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      updateUserPoolClient('eu-west-1_abc123', 'client-1', {
        clientName: 'web',
        explicitAuthFlows: [],
        allowedOAuthFlows: [],
        allowedOAuthScopes: [],
    ...
    ).rejects.toThrow('Cognito user pool client update request failed with status 400');
  });
});
...
describe('deleteUserPoolClient', () => {
  ...
  it('deletes the client with encoded ids', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();

    await deleteUserPoolClient('eu-west-1_abc 123', 'client 1', controller.signal);
...
describe('testHttpApiRoute', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it('posts the test-invoke request and returns the invocation result', async () => {
    const invocation = {
      statusCode: 200,
      authorized: true,
    ...
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => invocation });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
      stage: '$default',
    ...
    const result = await testHttpApiRoute('abc 123', request, controller.signal);

    expect(result.statusCode).toBe(200);
    expect(result.authorized).toBe(true);
    expect(result.latencyMilliseconds).toBe(12);
    expect(result.body).toBe('{"ok":true}');
    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/apigatewayv2/apis/abc%20123/routes/test-invoke',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(request),
  ...
  it('throws when the response is not ok', async () => {
    ...
    ).rejects.toThrow('API Gateway v2 route test-invoke request failed with status 400');
  });
});
...
describe('updateHttpIntegration', () => {
  ...
  it('puts the integration update request with the encoded ids', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();
    const request = {
      ...
      integrationUri: 'https://example.test/v2',
      payloadFormatVersion: '2.0',
      description: 'updated',
    };

    await updateHttpIntegration('abc 123', 'int 1', request, controller.signal);

    expect(fetchMock).toHaveBeenCalledWith(
      '/api/services/apigatewayv2/apis/abc%20123/integrations/int%201',
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(request),
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(
      updateHttpIntegration('abc123', 'int1', {
        integrationType: 'HTTP_PROXY',
        integrationMethod: null,
        integrationUri: null,
        payloadFormatVersion: null,
    ...
    ).rejects.toThrow('API Gateway v2 integration update request failed with status 400');
  });
});
...
describe('deleteHttpIntegration', () => {
  ...
  it('deletes the integration with the encoded ids', async () => {
    const fetchMock = vi.fn().mockResolvedValue({ ok: true });
    vi.stubGlobal('fetch', fetchMock);
    const controller = new AbortController();

    await deleteHttpIntegration('abc 123', 'int 1', controller.signal);
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(deleteHttpIntegration('abc123', 'missing')).rejects.toThrow(
      'API Gateway v2 integration delete request failed with status 404',
    );
  });
});
...
describe('exportWorkspaceSnapshot', () => {
  ...
  it('returns the parsed export result when the request succeeds', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        snapshotId: 'snap-1',
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(exportWorkspaceSnapshot()).rejects.toThrow(
      'Snapshot export request failed with status 500',
    );
  });
});

describe('importWorkspaceSnapshot', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('posts the snapshot and returns the parsed import result', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({
        operationId: 'op-1',
    ...
    expect(fetchMock).toHaveBeenCalledWith('/api/snapshot/import', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  it('throws when the response is not ok', async () => {
    ...
    await expect(importWorkspaceSnapshot(snapshot)).rejects.toThrow(
      'Snapshot import request failed with status 500',
    );
  });
});

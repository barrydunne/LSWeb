...
namespace Foundation.UnitTests.Application.Commands.DeleteHttpRoute;

public class DeleteHttpRouteCommandHandlerTests
{
    ...
    private DeleteHttpRouteCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteHttpRouteCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteRouteAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteHttpRouteCommand("abc123", "route1"), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).DeleteRouteAsync("abc123", "route1", Arg.Any<CancellationToken>());

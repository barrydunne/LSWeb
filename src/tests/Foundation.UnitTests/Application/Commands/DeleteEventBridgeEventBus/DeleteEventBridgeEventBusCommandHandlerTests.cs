...
public class DeleteEventBridgeEventBusCommandHandlerTests
{
    private readonly IEventBridgeClient _client = Substitute.For<IEventBridgeClient>();
    private readonly INotificationPublisher _publisher = Substitute.For<INotificationPublisher>();
    private readonly IActivityLog _activityLog = Substitute.For<IActivityLog>();

    private DeleteEventBridgeEventBusCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog,
            NullLogger<DeleteEventBridgeEventBusCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndLogsActivity()
    {
        // Arrange
        _client
            .DeleteEventBusAsync("orders-bus", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteEventBridgeEventBusCommand("orders-bus"), TestContext.Current.CancellationToken);

        // Assert
        result.IsSuccess.Should().BeTrue();
        await _client.Received(1).DeleteEventBusAsync("orders-bus", Arg.Any<CancellationToken>());

...
internal sealed partial class UpdateHttpStageCommandValidator : AbstractValidator<UpdateHttpStageCommand>
{
    private const int MaxStageNameLength = 128;

    private readonly ILogger _logger;

    public UpdateHttpStageCommandValidator(ILogger<UpdateHttpStageCommandValidator> logger)
    {
        _logger = logger;
    ...
    public override async Task<ValidationResult> ValidateAsync(
        ValidationContext<UpdateHttpStageCommand> context, CancellationToken cancellation = default)
    {
    ...
    [LoggerMessage(LogLevel.Warning, "UpdateHttpStageCommand validation failure: {Error}")]

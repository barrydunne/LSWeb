...
namespace Foundation.UnitTests.Application.Commands.DeleteRestStage;

public class DeleteRestStageCommandHandlerTests
{
    ...
    private DeleteRestStageCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteRestStageCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteStageAsync("api-1", "dev", Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        var result = await sut.Handle(
            new DeleteRestStageCommand("api-1", "dev"), TestContext.Current.CancellationToken);

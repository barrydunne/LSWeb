...
namespace Foundation.UnitTests.Application.Commands.CreateRestDeployment;

public class CreateRestDeploymentCommandHandlerTests
{
    ...
    private static CreateRestDeploymentCommand BuildCommand()
        => new("api-1", "dev", "Initial deployment");

    private CreateRestDeploymentCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreateRestDeploymentCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreateDeploymentAsync(Arg.Any<RestDeploymentSpecification>(), Arg.Any<CancellationToken>())
        ...
        result.Value.Should().Be("deployment-9");
        await _client.Received(1).CreateDeploymentAsync(
            Arg.Is<RestDeploymentSpecification>(specification =>
                specification.RestApiId == "api-1"
                && specification.StageName == "dev"
                && specification.Description == "Initial deployment"),

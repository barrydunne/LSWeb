...
{
    private const string Document = "{\"Version\":\"2012-10-17\",\"Statement\":[]}";
    ...
    private static CreatePolicyCommand BuildCommand()
        => new("deploy-policy", Document, "Deploy policy", "/team/");

    private CreatePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreatePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreatePolicyAsync("deploy-policy", Document, "Deploy policy", "/team/", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).CreatePolicyAsync(
            "deploy-policy", Document, "Deploy policy", "/team/", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenCreateFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .CreatePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())

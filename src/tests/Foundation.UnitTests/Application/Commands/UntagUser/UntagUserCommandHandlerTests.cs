...
public class UntagUserCommandHandlerTests
{
    ...
    private static UntagUserCommand BuildCommand()
        => new("alice", ["env"]);

    private UntagUserCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UntagUserCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UntagUserAsync("alice", Arg.Any<IReadOnlyList<string>>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UntagUserAsync(
            "alice", Arg.Any<IReadOnlyList<string>>(), Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .UntagUserAsync(Arg.Any<string>(), Arg.Any<IReadOnlyList<string>>(), Arg.Any<CancellationToken>())

...
public class UpdateRoleCommandHandlerTests
{
    ...
    private static UpdateRoleCommand BuildCommand()
        => new("deploy-role", "Deploy role", 7200);

    private UpdateRoleCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateRoleCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateRoleAsync("deploy-role", "Deploy role", 7200, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateRoleAsync(
            "deploy-role", "Deploy role", 7200, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenUpdateFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .UpdateRoleAsync(Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<int?>(), Arg.Any<CancellationToken>())

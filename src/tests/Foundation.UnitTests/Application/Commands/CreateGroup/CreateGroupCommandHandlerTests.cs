...
public class CreateGroupCommandHandlerTests
{
    ...
    private CreateGroupCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreateGroupCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreateGroupAsync("developers", "/team/", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).CreateGroupAsync("developers", "/team/", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenCreateFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .CreateGroupAsync(Arg.Any<string>(), Arg.Any<string?>(), Arg.Any<CancellationToken>())

...
public class UpdateParameterValueCommandHandlerTests
{
    ...
    private static UpdateParameterValueCommand BuildCommand()
        => new("/app/config/key", "new-value");

    private UpdateParameterValueCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateParameterValueCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .PutParameterValueAsync(Arg.Any<ParameterValueSpecification>(), Arg.Any<CancellationToken>())
    ...
    [Fact]
    public async Task Handle_MapsCommandFieldsOntoSpecification()
    {
        // Arrange
        ParameterValueSpecification? captured = null;
        _client
            .PutParameterValueAsync(
                Arg.Do<ParameterValueSpecification>(spec => captured = spec), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var sut = CreateSut();

        // Act
        await sut.Handle(new UpdateParameterValueCommand("/app/config/key", "new-value"), TestContext.Current.CancellationToken);

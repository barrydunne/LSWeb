...
namespace Foundation.Application.Commands.UpdateSchedule;

internal sealed partial class UpdateScheduleCommandHandler : ICommandHandler<UpdateScheduleCommand>
{
    private const string OperationName = "scheduler-update-schedule";
    ...
    public async Task<Result> Handle(UpdateScheduleCommand request, CancellationToken cancellationToken)
    {
        ...
        await _publisher.PublishAsync(
            new Notification(operationId, OperationName, OperationState.InProgress,
                $"Updating {request.GroupName}/{request.Name}.", DateTimeOffset.UtcNow),
            cancellationToken);
        ...
        if (!result.IsSuccess)
        {
            var failure = $"Failed to update {request.GroupName}/{request.Name}: {result.Error!.Value.Message}";
            await PublishOutcomeAsync(operationId, OperationState.Failed, failure, cancellationToken);
            return result.Error!.Value;
        }

        var message = $"Updated {request.GroupName}/{request.Name}.";
    ...
    [LoggerMessage(LogLevel.Trace, "Updating schedule {Group}/{Name}.")]
    private partial void LogHandling(string group, string name);

    [LoggerMessage(LogLevel.Trace, "Schedule update handled.")]

...
public class DeleteParameterCommandHandlerTests
{
    ...
    private DeleteParameterCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteParameterCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteParameterAsync("/app/config/key", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteParameterAsync("/app/config/key", Arg.Any<CancellationToken>());

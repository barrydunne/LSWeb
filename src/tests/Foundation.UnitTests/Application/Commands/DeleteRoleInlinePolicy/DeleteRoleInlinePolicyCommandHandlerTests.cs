...
public class DeleteRoleInlinePolicyCommandHandlerTests
{
    ...
    private static DeleteRoleInlinePolicyCommand BuildCommand()
        => new("deploy-role", "inline");

    private DeleteRoleInlinePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteRoleInlinePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteRoleInlinePolicyAsync("deploy-role", "inline", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteRoleInlinePolicyAsync("deploy-role", "inline", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenDeleteFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteRoleInlinePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

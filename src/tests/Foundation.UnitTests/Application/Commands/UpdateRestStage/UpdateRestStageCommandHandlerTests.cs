...
public class UpdateRestStageCommandHandlerTests
{
    ...
    private static UpdateRestStageCommand BuildCommand()
        => new(
            "api-1",
            "dev",
            "Updated description",
    ...
    private UpdateRestStageCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateRestStageCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateStageAsync(Arg.Any<RestStageSpecification>(), Arg.Any<CancellationToken>())

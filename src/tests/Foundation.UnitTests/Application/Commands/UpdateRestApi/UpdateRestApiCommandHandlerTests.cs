...
public class UpdateRestApiCommandHandlerTests
{
    ...
    private static UpdateRestApiCommand BuildCommand()
        => new("abc123", "orders-v2", "Updated description");

    private UpdateRestApiCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateRestApiCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateRestApiAsync(Arg.Any<RestApiSpecification>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateRestApiAsync(
            Arg.Is<RestApiSpecification>(specification =>
                specification.Id == "abc123"
                && specification.Name == "orders-v2"
                && specification.Description == "Updated description"),

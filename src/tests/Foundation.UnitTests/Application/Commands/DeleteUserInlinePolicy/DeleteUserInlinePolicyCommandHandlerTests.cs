...
public class DeleteUserInlinePolicyCommandHandlerTests
{
    ...
    private static DeleteUserInlinePolicyCommand BuildCommand()
        => new("alice", "inline");

    private DeleteUserInlinePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteUserInlinePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteUserInlinePolicyAsync("alice", "inline", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteUserInlinePolicyAsync("alice", "inline", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteUserInlinePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

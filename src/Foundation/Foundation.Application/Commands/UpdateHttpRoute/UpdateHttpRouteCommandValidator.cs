...
internal sealed partial class UpdateHttpRouteCommandValidator : AbstractValidator<UpdateHttpRouteCommand>
{
    private const int MaxRouteKeyLength = 256;

    private readonly ILogger _logger;

    public UpdateHttpRouteCommandValidator(ILogger<UpdateHttpRouteCommandValidator> logger)
    {
        _logger = logger;
    ...
    public override async Task<ValidationResult> ValidateAsync(
        ValidationContext<UpdateHttpRouteCommand> context, CancellationToken cancellation = default)
    {
    ...
    [LoggerMessage(LogLevel.Warning, "UpdateHttpRouteCommand validation failure: {Error}")]

...
public class UpdateUserPoolClientCommandHandlerTests
{
    ...
    private static UpdateUserPoolClientCommand BuildCommand()
        => new("eu-west-1_abc123", "client-1", "web", ["ALLOW_USER_SRP_AUTH"], ["code"], ["openid"], ["https://app/callback"], true);

    private UpdateUserPoolClientCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateUserPoolClientCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateUserPoolClientAsync(Arg.Any<Foundation.Domain.Cognito.UserPoolClientSpecification>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateUserPoolClientAsync(
            Arg.Is<Foundation.Domain.Cognito.UserPoolClientSpecification>(specification =>
                specification.ClientId == "client-1"

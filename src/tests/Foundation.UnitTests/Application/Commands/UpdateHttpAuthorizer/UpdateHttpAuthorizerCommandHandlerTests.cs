...
public class UpdateHttpAuthorizerCommandHandlerTests
{
    ...
    private static UpdateHttpAuthorizerCommand BuildCommand()
        => new(
            "abc123",
            "auth1",
            "jwt-authorizer",
    ...
    private UpdateHttpAuthorizerCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateHttpAuthorizerCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateAuthorizerAsync(Arg.Any<HttpAuthorizerSpecification>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateAuthorizerAsync(
            Arg.Is<HttpAuthorizerSpecification>(specification =>
                ...
                && specification.AuthorizerId == "auth1"
    ...
    public async Task Handle_MapsAllCommandFieldsOntoSpecification()
    {
        ...
        _client
            .UpdateAuthorizerAsync(Arg.Any<HttpAuthorizerSpecification>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var command = new UpdateHttpAuthorizerCommand(
            "api9",
            "auth9",
            "name9",
            "JWT",

...
public class UpdateRoleTrustPolicyCommandHandlerTests
{
    private const string PolicyDocument = "{\"Version\":\"2012-10-17\",\"Statement\":[]}";
    ...
    private static UpdateRoleTrustPolicyCommand BuildCommand()
        => new("deploy-role", PolicyDocument);

    private UpdateRoleTrustPolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateRoleTrustPolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateRoleTrustPolicyAsync("deploy-role", PolicyDocument, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateRoleTrustPolicyAsync(
            "deploy-role", PolicyDocument, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenUpdateFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .UpdateRoleTrustPolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

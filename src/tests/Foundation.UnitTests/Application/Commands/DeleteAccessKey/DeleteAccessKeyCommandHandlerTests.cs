...
public class DeleteAccessKeyCommandHandlerTests
{
    ...
    private static DeleteAccessKeyCommand BuildCommand()
        => new("alice", "AKIAEXAMPLE");

    private DeleteAccessKeyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteAccessKeyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteAccessKeyAsync("alice", "AKIAEXAMPLE", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteAccessKeyAsync("alice", "AKIAEXAMPLE", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteAccessKeyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

...
public class UpdateHttpIntegrationCommandHandlerTests
{
    ...
    private static UpdateHttpIntegrationCommand BuildCommand()
        => new("abc123", "int1", "HTTP_PROXY", "GET", "https://example.test", "1.0", "proxy");

    private UpdateHttpIntegrationCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateHttpIntegrationCommandHandler>.Instance);
    ...
    public async Task Handle_WhenUpdateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateIntegrationAsync(Arg.Any<HttpIntegrationSpecification>(), Arg.Any<CancellationToken>())
    ...
    public async Task Handle_MapsAllCommandFieldsOntoSpecification()
    {
        ...
        _client
            .UpdateIntegrationAsync(Arg.Any<HttpIntegrationSpecification>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var command = new UpdateHttpIntegrationCommand(
            "api9", "int9", "AWS_PROXY", "POST", "arn:lambda", "2.0", "lambda proxy");
        var sut = CreateSut();
        ...
        // Assert
        await _client.Received(1).UpdateIntegrationAsync(
            Arg.Is<HttpIntegrationSpecification>(specification =>
                specification.ApiId == "api9"
                && specification.IntegrationId == "int9"

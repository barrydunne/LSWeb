...
public class PutGroupInlinePolicyCommandHandlerTests
{
    private const string PolicyDocument = "{\"Version\":\"2012-10-17\"}";
    ...
    private static PutGroupInlinePolicyCommand BuildCommand()
        => new("developers", "inline", PolicyDocument);

    private PutGroupInlinePolicyCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<PutGroupInlinePolicyCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .PutGroupInlinePolicyAsync("developers", "inline", PolicyDocument, Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).PutGroupInlinePolicyAsync(
            "developers", "inline", PolicyDocument, Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .PutGroupInlinePolicyAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())

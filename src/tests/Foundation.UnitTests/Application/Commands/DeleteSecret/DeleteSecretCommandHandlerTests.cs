...
public class DeleteSecretCommandHandlerTests
{
    ...
    private DeleteSecretCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteSecretCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteSecretAsync("db-password", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteSecretAsync("db-password", Arg.Any<CancellationToken>());

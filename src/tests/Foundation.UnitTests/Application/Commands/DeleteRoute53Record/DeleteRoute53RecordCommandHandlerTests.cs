...
public class DeleteRoute53RecordCommandHandlerTests
{
    private readonly IRoute53Client _client = Substitute.For<IRoute53Client>();
    private readonly INotificationPublisher _publisher = Substitute.For<INotificationPublisher>();
    private readonly IActivityLog _activityLog = Substitute.For<IActivityLog>();

    private DeleteRoute53RecordCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, NullLogger<DeleteRoute53RecordCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccess()
    {
        // Arrange
        _client
            .DeleteRecordAsync("/hostedzone/Z1", Arg.Any<Route53Record>(), Arg.Any<CancellationToken>())

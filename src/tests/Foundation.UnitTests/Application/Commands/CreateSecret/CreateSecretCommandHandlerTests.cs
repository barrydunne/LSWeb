...
public class CreateSecretCommandHandlerTests
{
    ...
    private static CreateSecretCommand BuildCommand()
        => new("db-password", "primary db", "s3cr3t");

    private CreateSecretCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreateSecretCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreateSecretAsync(Arg.Any<SecretSpecification>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).CreateSecretAsync(
            Arg.Is<SecretSpecification>(spec => spec.Name == "db-password"),
    ...
    public async Task Handle_MapsAllCommandFieldsOntoSpecification()
    {
        ...
        _client
            .CreateSecretAsync(Arg.Any<SecretSpecification>(), Arg.Any<CancellationToken>())
            .Returns(Task.FromResult(Result.Success()));
        var command = new CreateSecretCommand("db-password", "primary db", "s3cr3t");
        var sut = CreateSut();
        ...
        // Assert
        await _client.Received(1).CreateSecretAsync(
            Arg.Is<SecretSpecification>(spec =>
                spec.Name == "db-password"
                && spec.Description == "primary db"
                && spec.SecretString == "s3cr3t"),
            Arg.Any<CancellationToken>());
    }
}

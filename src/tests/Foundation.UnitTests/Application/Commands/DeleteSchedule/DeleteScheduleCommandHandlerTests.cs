...
public class DeleteScheduleCommandHandlerTests
{
    ...
    private static DeleteScheduleCommand BuildCommand()
        => new("daily-job", "default");

    private DeleteScheduleCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteScheduleCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteScheduleAsync("daily-job", "default", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteScheduleAsync("daily-job", "default", Arg.Any<CancellationToken>());

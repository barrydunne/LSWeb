...
public class DeleteGroupCommandHandlerTests
{
    ...
    private static DeleteGroupCommand BuildCommand()
        => new("developers");

    private DeleteGroupCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<DeleteGroupCommandHandler>.Instance);
    ...
    public async Task Handle_WhenDeleteSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .DeleteGroupAsync("developers", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).DeleteGroupAsync("developers", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenDeleteFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .DeleteGroupAsync(Arg.Any<string>(), Arg.Any<CancellationToken>())

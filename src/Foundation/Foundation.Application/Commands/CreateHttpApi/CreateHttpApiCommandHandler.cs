...
internal sealed partial class CreateHttpApiCommandHandler : ICommandHandler<CreateHttpApiCommand, string>
{
    private const string OperationName = "apigatewayv2-create-api";
    ...
    public CreateHttpApiCommandHandler(
        IApiGatewayV2Client client,
        INotificationPublisher publisher,
        IActivityLog activityLog,
        ISearchRefreshTrigger searchRefresh,
        ILogger<CreateHttpApiCommandHandler> logger)
    {
        _client = client;
    ...
    public async Task<Result<string>> Handle(CreateHttpApiCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await _client.CreateApiAsync(specification, cancellationToken);
        if (!result.IsSuccess)
        {
    ...
    [LoggerMessage(LogLevel.Trace, "Creating API Gateway v2 API {Name}.")]
    private partial void LogHandling(string name);

    [LoggerMessage(LogLevel.Trace, "API Gateway v2 create API handled.")]

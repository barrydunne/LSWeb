...
public class CreateRestAuthorizerCommandHandlerTests
{
    ...
    private static CreateRestAuthorizerCommand BuildCommand()
        => new(
            "api-1",
            "pool-authorizer",
            "COGNITO_USER_POOLS",
    ...
    private CreateRestAuthorizerCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<CreateRestAuthorizerCommandHandler>.Instance);
    ...
    public async Task Handle_WhenCreateSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .CreateAuthorizerAsync(Arg.Any<RestAuthorizerSpecification>(), Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).CreateAuthorizerAsync(
            Arg.Is<RestAuthorizerSpecification>(specification =>
                ...
                && specification.Type == "COGNITO_USER_POOLS"
                && specification.ProviderARNs.Count == 1),

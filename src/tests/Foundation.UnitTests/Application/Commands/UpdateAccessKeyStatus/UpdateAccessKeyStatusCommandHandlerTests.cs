...
public class UpdateAccessKeyStatusCommandHandlerTests
{
    ...
    private static UpdateAccessKeyStatusCommand BuildCommand()
        => new("alice", "AKIAEXAMPLE", "Inactive");

    private UpdateAccessKeyStatusCommandHandler CreateSut()
        => new(_client, _publisher, _activityLog, _searchRefresh, NullLogger<UpdateAccessKeyStatusCommandHandler>.Instance);
    ...
    public async Task Handle_WhenSucceeds_PublishesSuccessAndRefreshesSearch()
    {
        // Arrange
        _client
            .UpdateAccessKeyStatusAsync("alice", "AKIAEXAMPLE", "Inactive", Arg.Any<CancellationToken>())
        ...
        await _client.Received(1).UpdateAccessKeyStatusAsync(
            "alice", "AKIAEXAMPLE", "Inactive", Arg.Any<CancellationToken>());
    ...
    public async Task Handle_WhenFails_PublishesFailureAndReturnsError()
    {
        // Arrange
        _client
            .UpdateAccessKeyStatusAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(), Arg.Any<CancellationToken>())
